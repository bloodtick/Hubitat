...
def webSocketStatus(String message) {
    if(debugLevel == "Full") log.debug("${device.displayName} webSocketStatus $message${state?.webSocketTimestamp ? " after ${(now() - state?.webSocketTimestamp)/1000 as Integer} seconds" : ""}")    
    if(message?.contains("open")) {
        if(debugLevel != "None" && state?.webSocketOpenDelay!=1) log.info "<b>${device.displayName} websocket open</b>"
...
void webSocketProcess(data) {
    final BigDecimal toPSI = 14.503773773
	...
	data?.each {
        if(it?.value==null || it.value?.invalid?.toBoolean()!=true) { // toss any invalid data first
