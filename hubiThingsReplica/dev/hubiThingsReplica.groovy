...
*  1.4.00 2024-07-20 Intial support for Home Assistant replica devices. Requires replica.hass drivers to enable.
*  LINE 30 MAX */ 
...
@Field volatile static Map<Long,List>  g_mHassDeviceList = [:]
...
void clearAllVolatileCache() {
    ...
    g_mHassDeviceList[appId]?.clear()
    g_mHassDeviceList[appId]=null
}
...
public List getSubscriptionList(String apiId) {
    ...
    getAllHassReplicaDevices()?.each{ replicaDevice ->
        ...
        if(apiId==smartDevice?.apiId && smartDevice?.entity_ids[apiId]) subscriptions.addAll( smartDevice?.entity_ids[apiId] )
    }      
    return subscriptions.unique()
}

public void parseMessage(def message) {
    ...
    smartDevice?.entity_ids?.each{ apiId, entity_ids -> 
        ...
        entity_ids?.each{ entity_id ->
            ...
            if(service=="refresh") { // special call to shift from websocket to rest api for status                
                getChildDeviceHassAPI( apiId )?.getStatus( entity_id )
                runIn(2, refreshHassAPIs)
                return
            }
            ...
            getChildDeviceHassAPI( apiId )?.sendMessage( message, true, callback )
        }
    }
}
...
void refreshHassAPIs(String apiId = null) {
    ...
    getChildDeviceHassAPIs()?.each{
        if(apiId==null || it?.getApiId()==apiId) it.refresh()
    }
}    

def getChildDeviceHassAPIs() {
    return getChildDevices()?.findAll{ it.typeName==sApiTypeName }
}

def getChildDeviceHassAPI(String apiId) {
    return getChildDeviceHassAPIs()?.find{ it.getApiId() == apiId }
}
...
def pageMainCreateChildDeviceHassAPI() {
    ...
    runIn(3, 'allSmartDeviceRefresh')
}

List getHassDeviceList() {
    Long appId = app.getId()
    if(g_mHassDeviceList[appId]==null) {
        g_mHassDeviceList[appId]=[]
        logInfo "${app.getLabel()} caching Home Assistant device list"
        getChildDeviceHassAPIs()?.each{
            g_mHassDeviceList[appId].addAll( it?.getDeviceList() ?: [] )
        }      
    }
    return g_mHassDeviceList[appId]?:[]
}
...
Map getHassCustomDevice(String deviceId) {
    ...
    getReplicaDevices(deviceId)?.each { replicaDevice ->
        ...
        if(replica?.namespace==sHassNamespace && replica?.containsKey("deviceIds")) {
            ...
            replica?.deviceIds?.each { nestDeviceId ->
                Map nestSmartDevice = getHassDeviceList()?.find{ it.deviceId==nestDeviceId }?.clone()
    ...
    if(smartDevice != [:]) g_mHassDeviceList[app.getId()] << smartDevice
    return smartDevice
}
...
Map getSmartDevice(String deviceId) {
    return (getSmartDevicesMap()?.items?.find{ it.deviceId==deviceId } ?: getHassDeviceList()?.find{ it.deviceId==deviceId } ?: getHassCustomDevice(deviceId)).clone()
}

def getSmartDeviceApi(Map smartDevice) {
    if(smartDevice && !smartDevice.isEmpty() && smartDevice?.namespace == sHassNamespace) 
        return getChildDeviceHassAPI(smartDevice?.apiId)
...
List getSmartThingsDeviceList() {    
    getSmartDevicesMap()?.items?.findAll{ it.namespace != sHassNamespace } ?: []
}
...
void setSmartDevicesMap(Map deviceList) {
    g_mSmartDeviceListCache[app.getId()]?.clear()
...
def pageMain(){
    ...
    List smartDevices = getSmartThingsDeviceList()?.clone()
    ...
    return dynamicPage(name: "pageMain", install: true, refreshInterval:iPageMainRefreshInterval) {
            ...
            if(pageMainShowConfig) {
                ...
                if(userShowHass && isHassInstalled) {                    
                    ...
                    if(userHassLLAT&&userHassIp&&userHassPort) {
                        ...
                        Boolean isDupHassApi = !!getChildDeviceHassAPIs()?.find{ Map hassAPI=it?.getConfig(); return(userHassIp==hassAPI?.hassIp && userHassPort as Integer==hassAPI?.hassPort as Integer) }
                        ...
                        getChildDeviceHassAPIs()?.each { hassAPI ->
                            ...
                            href(title: "$sHassIcon $config.locationName : ($config.uri)", url: config.uri, width: 6, description:"", newLineAfter:true)
                        }
                    }
                }
        ...
        if(getChildDeviceHassAPIs()?.size() || getChildApps().size()) {            
			section(menuHeader("HubiThings Device List")){           
				if (deviceCount) {
                    String platformIcon = getChildApps()?.size()&&getChildDeviceHassAPIs()?.size() ? "$sSamsungIcon || $sHassIcon" : getChildApps()?.size() ? sHubitatIcon : sHassIcon
			...
			section(menuHeader("Replica Device Creation and Control")){
                if(getChildApps().size()) {
                    if(getChildDeviceHassAPIs().size()) paragraph("<b>$sSamsungIcon SmartThings:</b>")
                ...
                if(getChildDeviceHassAPIs().size()) {
...
def pageHubiThingDevice() {
    ...
    List smartDevices = (dynamicPageName=="pageHubiThingDeviceHass") ? getHassDeviceList() : getSmartThingsDeviceList() 
...
void pageHubiThingDeviceCreateButton() {
    ...
    List smartDevices = (state?.dynamicPageName=="pageHubiThingDeviceHass") ? getHassDeviceList() : getSmartThingsDeviceList()
...
def pageMirrorDevice(){
    ...
    List smartDevices = getSmartThingsDeviceList()
...
void allSmartDeviceRefresh(delay=1) {
    ...
    getHassDeviceList()?.each{
        if( getReplicaDevices(it?.deviceId) ) smartDevices.items.add( it )
    }
...
String updateSmartDeviceEventsStatus(replicaDevice) {
    ...
    if(replicaDevice) { 
        ...
        String noRules = (getReplicaDataJsonValue(replicaDevice, "rules")?.components || isHass) ? "" : "<span style='color:$sColorDarkRed;'>$sWarningsIcon $sNoRules</span>"
