...
metadata 
{
    definition(name: "Replica Scene Knob", namespace: "replica", author: "bloodtick", importUrl:"https://raw.githubusercontent.com/bloodtick/Hubitat/main/hubiThingsReplica/devices/replicaSceneKnob.groovy")
    {
        capability "Actuator"
...
Map getReplicaCommands() {
    return ([ "setSceneIdValue":[[name:"sceneId*",type:"STRING"]], "setSwitchOff":[], "setSwitchOn":[], "setHealthStatusValue":[[name:"healthStatus*",type:"ENUM"]]])
}

def setSceneIdValue(value) {
    state.sceneId = value
    refresh()
}
...
def watchdog() {
    logWarn "${device.displayName} did not receive a timely response from SmartThings. Please ensure the corresponding SmartThings Virtual Switch is included in the scene actions and it updates to 'Turn on' when the scene is triggered."
    setSwitchOn()
    runIn(2,setSwitchOff)
}
...
def on() {
    if( triggerScene(state?.sceneId)?.statusCode == iHttpSuccess )
        runIn(5, watchdog)
}
...
Map triggerScene(String sceneId) {
    logDebug "${device.displayName} executing 'triggerScene($sceneId)'"
    ...
    try {
        httpPost(params) { resp ->
            ...
            logInfo "${device.displayName} executed SmartThings scene"
        }
    } catch (e) {
        logWarn "${device.displayName} has triggerScene() error: $e"        
    }
    return response
}
